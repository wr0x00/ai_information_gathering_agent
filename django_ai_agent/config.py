...
class AIModelConfig:
    ...
    def _load_models(self) -> Dict:
        """Load AI model configurations from YAML file"""
        if os.path.exists(self.config_file):
            with open(self.config_file, 'r', encoding='utf-8') as f:
    ...
    def get_enabled_models(self) -> List[str]:
        ...
        enabled_models = []
        for provider, provider_config in self.models.items():
            models = provider_config.get('models', [])
            for model in models:
                if model.get('enabled', False):
                    enabled_models.append(f"{provider}:{model['name']}")
    ...
    def get_model_config(self, provider: str, model_name: str) -> Dict[str, Any]:
        ...
        # Return provider-level config plus model-specific info
        config = {
